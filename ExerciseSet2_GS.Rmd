...
**Your Answers here:**
Q1. We need at least 6 principal components (Cumulative_percentage_of_explained_variance = 52.29%)
...
Q3.
Component1: Bonds
Component2: Currencies
Component3: European Stockmarket
Component4: North American Stockmarket
Component5: European Bonds
Component6: Oil
Component7: Minerals
Component8: Grain
Component9: Precious metal
Component10: Asian Stockmarket

Q4. 

a)

To generate the principal components and scree plot using only the pre-crisis bull market years (e.g. only using the data between November 1, 2002, and October 1, 2007) we will add  the following lines of code in the chunck of line 74:
'futures_data_original<-futures_data' #creates a copy of the futures_data for back up purposes

'futures_data = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01", ] '

The above line keeps only the data between November 1, 2002, and October 1, 2007 in the futures_data table

Running the rest of the code as it is generates the  principal components and scree plot using only the pre-crisis bull market years

b)

To generate the principal components and scree plot using only the financial crisis years (e.g. only using the data between October 1, 2007 and  March 1, 2009) we we will add  the following lines of code in the chunck of line 74:
'futures_data_original<-futures_data' #creates a copy of the futures_data for back up purposes

'futures_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ] '

Q5. 

Comparing the pre-crisis and crisis data we notice that while pre-crisis there is a larger number of factors to explain 50% of the variance: 
 - US Bonds
 - Currencies
 - European Bonds
 - Minerals
...
On the other hand, during the crisis the variance (up to 50%) can be explained using only 4 factors:
 - US Bonds
 - Currencies
 - Grain
 - North American Stockmarkets
 
This means that during crisis, the return of the futures are highly correlated and driven by the U Bonds and currencies (as expected) but also the grain commodities, because financial crisis puts pressure on the prices of these commodities which become more sensitive to the economy compared to the pre-crisis years. 
...
<br>
1. 
The code first creates a zero table ‘signal used’ that has the same dimensions as ‘future_data’ .

Then, the code iterated through the length of the vector MAfreq and based on the moving average of the futures data at specified intervals (based on MAfreq) it sets either a positive or negative sign on the table signal_used’.

Next, the code divides each item of table ‘signal_used’ with the sum of all values for each column (if different than zero), and transposed the result.

Then, the code takes the value from 2 days ago of the ‘signal_used’ table and multiplies with the original value of the ‘futures data’. 

Last, the code sums the values of each row and plots them against the date

2.

When we try different moving average technical indicators, the results are different, but the differences are very small, meaning that the final estimation is not heavily affected by the moving average technique.
